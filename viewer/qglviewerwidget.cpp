...
const OpenMesh::Vec3f& QGLViewerWidget::Center(void) const
{
    return center;
}
...
{
    //std::cout<<"SetProjectionMode"<<std::endl;
    projectionmode = pm;
    UpdateProjectionMatrix();
    //std::cout<<"SetProjectionMode->ViewAll"<<std::endl;
    ViewAll();
}
...
void QGLViewerWidget::initializeGL(void)
{
    ...
    glGetDoublev(GL_MODELVIEW_MATRIX, &modelviewmatrix[0]);
    CopyModelViewMatrix();
    //std::cout<<"QGLViewerWidget::initializeGL->SetScenePosition"<<std::endl;
    SetScenePosition(OpenMesh::Vec3f(0.0, 0.0, 0.0), 1.0);
    //LoadTexture();
}
...
void QGLViewerWidget::Translation(const QPoint & p)
{
    ...
    Translate(OpenMesh::Vec3f(2.0*dx / w*right / near_plane*z,
...
void QGLViewerWidget::Rotation(const QPoint & p)
{
    ...
    {
        OpenMesh::Vec3f axis = lastpoint3 % newPoint3D;
        ...
        OpenMesh::Vec3f d = lastpoint3 - newPoint3D;
...
void QGLViewerWidget::Rotate(const OpenMesh::Vec3f& _axis, const double & _angle)
{
    // Rotate around center center_, axis _axis, by angle _angle
    // Update modelview_matrix_

    OpenMesh::Vec3f t(modelviewmatrix[0] * center[0] +
...
bool QGLViewerWidget::MapToSphere(const QPoint& _v2D, OpenMesh::Vec3f& _v3D)
{
...
void QGLViewerWidget::SetScenePosition(const OpenMesh::Vec3f& _center, const double & _radius)
{
    center = _center;
    radius = _radius;
...
{
    OpenMesh::Vec3f _trans = OpenMesh::Vec3f(-(modelviewmatrix[0] * center[0] +
        modelviewmatrix[4] * center[1] +
        modelviewmatrix[8] * center[2] +
        modelviewmatrix[12]),
        -(modelviewmatrix[1] * center[0] +
