...
//    set<int> boundarySet;
//    std::vector<vector<float>> edgeLengthss; // cumulative edge lengths
//    std::vector<float> totalEdgeLengths;
//    std::vector<vector<VertexIter>> bds;

//    std::map<std::pair<int,int>,bool> visitedMap;
//    for(HalfEdgeIter heit = hem.BeginHalfEdges(); heit != hem.EndHalfEdges(); heit++){
//        int i0=heit->vertex->id;
//        int i1=heit->next->vertex->id;
//        std::pair<int, int> heid(i0, i1);
//        if(visitedMap.count(heid)>0)continue;
//        visitedMap[heid]=true;
//        if(hem.IsBoundary(heit)) {
//            float totalEdgeLength = 0;
//            vector<float> edgeLengths;
//            vector<VertexIter> boundaryVertices;
//            HalfEdgeIter firstBoundary=heit;
//            HalfEdgeIter previousBoundary = firstBoundary;
//            HalfEdgeIter currentBoundary = firstBoundary;
//            do {
//                boundaryEdges.push_back(currentBoundary);
//                boundaryVertices.push_back(currentBoundary->vertex);
//                boundarySet.insert(currentBoundary->vertex->id);
...
//                previousBoundary = currentBoundary;
//            } while(currentBoundary != firstBoundary);
//            edgeLengthss.emplace_back(edgeLengths);
//            totalEdgeLengths.emplace_back(totalEdgeLength);
//            bds.emplace_back(boundaryVertices);
//        }
//    }
...
//        std::vector<VertexIter> boundaryVertices=bds[k];
//        float totalEdgeLength=totalEdgeLengths[k];
//        std::vector<float> edgeLengths=edgeLengthss[k];
//        for(int i = 0; i < boundaryVertices.size(); i++) {
//            VertexIter vit = boundaryVertices[i];
//            double theta = (edgeLengths[i]/totalEdgeLength)*2.0f*M_PI;
//            bx[vit->id] = cos(theta);
