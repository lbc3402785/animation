...
HalfEdgeIter HalfEdgeMesh::GetNextBoundary(HalfEdgeIter heit,std::map<std::pair<int,int>,bool>& visitedMap) {
    ...
    do {
        std::pair<int,int> heid(halfEdge->vertex->id,halfEdge->next->vertex->id);
...
HalfEdgeIter HalfEdgeMesh::GetNextBoundary(HalfEdgeIter heit) {
    ...
    do {
        ...
        halfEdge = halfEdge->twin->next;
        tmp.push_back(halfEdge->vertex->id);
    ...
    std::cout<<std::endl;
    std::cout<<(halfEdge==first)<<std::endl;
