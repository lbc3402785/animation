...
inline cv::Mat PoissonBlending(cv::Mat src, cv::Mat target, cv::Mat msk, bool UsePoisson,int dilation_size = 20,int dilation_size1 = 60, cv::Scalar* _diff = nullptr)
{
	using namespace std;
	using namespace cv;

	{
		...
		element = getStructuringElement(MORPH_ELLIPSE,
            ...
            Point(dilation_size1, dilation_size1));
		erode(msk, msk, element);
        GaussianBlur(msk, msk, Size(2 * dilation_size1 + 1, 2 * dilation_size1 + 1), dilation_size1 / 2);
