...
void KNNSearch::searchKNN(float *dst, int64_t dstRows, int64_t dstCols,int K)
{
    assert(dstCols==this->cols);
    srcIndex.clear();
    distances.clear();
    srcIndex.resize(dstRows);
    std::vector<int> tempIndex(dstRows*K,0);
    std::vector<float> tempDistances(dstRows*K,0.0);
    flann::Matrix<float> query(dst,dstRows,dstCols);
    flann::Matrix<int> indices(tempIndex.data(), dstRows, K);
    flann::Matrix<float> dists(tempDistances.data(), dstRows, K);
...
//    timedebuge.start();//开始计时
    ...
    flannIndex->knnSearch(query, indices, dists, K, param);
