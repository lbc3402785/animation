...
std::vector<int> MeshTools::getBoundaries(std::vector<float>& inVertices,std::vector<int>& inFaces,int firstBoundId){
    ...
    for(HalfEdgeIter heit = hem.BeginHalfEdges(); heit != hem.EndHalfEdges(); heit++) {
        if(hem.IsBoundary(heit)&&heit->vertex->id==firstBoundId) {
            firstBoundary = heit;
            break;
        }
    }
    //std::cout<<"find first id!"<<std::endl;
    if(firstBoundary == hem.EndHalfEdges()) {
        std::cout<<"ERROR: found no boundary in mesh"<<std::endl<<std::flush;
    }
    ...
    do {
        ...
        bds.emplace_back(currentBoundary->vertex->id);
        //std::cout<<currentBoundary->vertex->id<<std::endl;
...
bool MeshTools::ReadOBJ(Mesh & mesh, const std::string & filename)
{
    ...
    while (ifs && !ifs.eof())
    {
        ...
        if (keyWrd == "v")
        {
            ...
            if (!stream.fail())
            {
                mesh.set_texcoord2D(vertexHandles.back(), OpenMesh::Vec2f(x, y));
            }
        }
    }
